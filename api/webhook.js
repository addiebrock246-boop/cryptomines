...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!msg?.text?.startsWith('/start') || !msg.chat?.id) return res.status(200).json({ ok: true });
    ...
    await fetch(`${BASE_URL}/sendPhoto`, {
      ...
      body: JSON.stringify({
        chat_id: chatId,
        photo: PHOTO_URL,
        caption: "🎰 **RK | CryptoMines**\n▔▔▔▔▔▔▔▔▔▔▔▔▔▔\n💎 *Real Crypto Mines Game*\n⛏️ Find diamonds, avoid bombs\n💰 Win USDT by playing!\n📌 Minimum Deposit: 10 USDT\n🔗 BNB Smart Chain (BEP20)\n\n👇 Tap the button below to play!",
        parse_mode: 'Markdown',
        reply_markup: { inline_keyboard: [[{ text: "🎮 Launch Game", web_app: { url: GAME_URL } }]] }
      })
    });
